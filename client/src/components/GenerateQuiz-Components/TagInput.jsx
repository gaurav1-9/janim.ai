...
const TagInput = ({ addTags, removeTag, tags, input, inpErr, setInput, tagList, isGenerating, showMsg }) => {
    ...
    return (
        <div className='max-h-60 px-8 lg:pr-3 py-5 md:px-20 xl:pl-40 flex flex-col text-eerieBlack relative lg:w-6/11'>
            <p className={`text-sm lg:text-lg absolute z-10 -top-2 text-lightRed/70 font-semibold 
                ${(inpErr.isErr)
                    ? 'opacity-100 top-0'
                    : 'opacity-0 bg-seaSalt -top-2'
                } transition-all duration-300 ease-in-out`}>{errMsg[inpErr.errType]}</p>
